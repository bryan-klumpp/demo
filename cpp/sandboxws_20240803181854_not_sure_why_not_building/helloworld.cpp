...
enum MyType { enum1 = 2, enum2 = 4, enum3 = 8, ALL = enum1 & enum2 & enum3, last };

constexpr static const bool enumtest(const MyType& ALL, const MyType& lastBase, const MyType& last) {
    for ( int i = 2; i < lastBase; i = i * 2 ) {
        if ( i > ALL ) {
            return true;
        }
    }
...
static_assert( enumtest(MyType::ALL, MyType::enum3, MyType::last) );
