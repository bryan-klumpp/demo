...
enum MyType { enum1 = 2, enum2 = 4, enum3 = 8, ALL = enum1 + enum2 };
constexpr static bool enumtest(const MyType& ALL, const MyType& lastBase) {
    int i = 2;
    for ( i = 2; i < lastBase; i = i * 2 ) {
        if ( ALL < i ) {
            return false;
        }
    }
...
static_assert( enumtest(MyType::ALL, MyType::enum3) );
void enumfun() {
    cout << MyType::enum1;
    cout << MyType::ALL;
    cout << enumtest(MyType::ALL, MyType::enum3);
}
