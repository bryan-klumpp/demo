...
public class TimeTrackingCommand {

	public void exec(List<String> args, PrintWriter outputWriter) {
    	PrintWriter writer = outputWriter;
        if (args.isEmpty()) {
            sopInProgress(outputWriter);
            return;
        }
...
//        }
        ...
        //}
        writer.println(StringUtil.toStringMultiLine(Q.table("select * from v_do_log where i > (select max(i) - 10 from do_log) or i = ?", true, i)));
	 ...
	 * 
	 */
	protected void sopInProgressOld(PrintWriter outputWriter) {
        final List<List<Object>> inProgress = Q.table("select * from v_do_log where end = 'in_progress'", false);
        if (!inProgress.isEmpty()) {
            outputWriter.println("in progress........................");
            outputWriter.println(inProgress);
        } else {
            outputWriter.println("no active tasks");
        }
    }
    private void sopInProgress(PrintWriter outputWriter) {
        final List<List<Object>> inProgress = Q.table("select * from v_do_log where i > (select max(i) - 30 from v_do_log)", false);
        StringUtil.printAsLines(inProgress, outputWriter);
    }
