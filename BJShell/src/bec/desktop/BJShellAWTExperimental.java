...
import com.bryanklumpp.bjshell.BJShellState;
 ...
 *        modification.
 */
public class BJShellAWTExperimental extends Frame implements CommandContext {
	...
	private PrintWriter buildPrintWriter(TextArea ta2, ScrollPane sp2) {
	 ...
	 * @return
	 */
	StringWriter buildStringWriter(TextArea taLocal, ScrollPane spLocal) { 
		final TextArea syncMe = taLocal;
		return new StringWriter() {
			public void flush() {
				synchronized (syncMe) { // unlikely that synchronized vs. not is going to be critical here, but if it
										...
										// https://community.oracle.com/tech/developers/discussion/2170775/is-TextArea-append-method-thread-safe
					super.flush();
...
//		ta.addCaretListener(getCaretListener());
		...
		setVisible(true);
		getWriter().flush(); // forces scroll to the bottom of the TextArea in the case
		tf.requestFocusInWindow();
	}
