...
co "$1" '^x$' && {
  ...
  isval "$val" && test -L ${HOME}/l/$num && { echo "duplicate bln: "$num; rm ${HOME}/l/$num; }
  ln -s {} ${HOME}/l/$num || echo "something bad happened: $num "{};
  return
}
...
find $(can "$ddir") | grep -vE 'backup|copy\W|archive|RecentDocuments|wav_masters' |
   ...
   xargs -i -r --null bash --rcfile /tmp/dummyrc -r -c \
       ' { val={}; num=$(echo {} | sed --regexp-extended "s@.*/b([0-9]*)_[^/]*/?\$@\1@g"); 
           test -L ${HOME}/l/$num && { echo "duplicate bln: "$num; rm ${HOME}/l/$num; }
           ln -s {} ${HOME}/l/$num || echo "something bad happened: $num "{}; } ' 
